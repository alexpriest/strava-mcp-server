...
export async function createStreamableApp(
  ...
  // Hybrid resource auth: legacy AUTH_TOKEN OR OAuth bearer.
  //
  // FAILS CLOSED. This previously fell through to next() when neither was
  // configured, serving the endpoint to any unauthenticated caller. Observed
  // live on 2026-07-26 when a fresh deploy copied from this template came up
  // with no auth env vars and returned its full tool list over the public
  // internet. A misconfigured deploy must be inert, never open.
